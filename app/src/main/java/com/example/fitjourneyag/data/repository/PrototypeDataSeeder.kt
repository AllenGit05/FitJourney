...
import com.example.fitjourneyag.data.local.dao.*
import com.example.fitjourneyag.data.local.entity.*
import com.example.fitjourneyag.domain.model.Exercise
import com.example.fitjourneyag.domain.model.WorkoutSet
...
class PrototypeDataSeeder(
    ...
    private suspend fun seedHistoricalData() {
        ...
        for (i in 0..7) {
            ...
            stepsDao.insertSteps(StepsEntity(date = timestamp, count = stepCount))

            // Seed Weight (slightly fluctuating)
            val baseWeight = 75.0f
            val weight = baseWeight - (i * 0.1f) + (random.nextFloat() * 0.5f)
            weightDao.insertWeight(WeightEntity(date = timestamp, weight = weight))
