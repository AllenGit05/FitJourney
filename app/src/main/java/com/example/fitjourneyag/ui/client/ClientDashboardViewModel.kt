package com.example.fitjourneyag.ui.client
...
class ClientDashboardViewModel(
    ...
    private val authRepository: com.example.fitjourneyag.domain.repository.AuthRepository,
    private val userRepository: com.example.fitjourneyag.domain.repository.UserRepository,
    ...
    private fun generateDailyGreetingAI() {
        ...
        viewModelScope.launch {
            try {
                val configs = apiRepository.apiConfigs.first()
                val groq = configs.find { it.providerName == "Groq" && it.isActive && it.apiKey.isNotEmpty() }
                val gemini = configs.find { it.providerName == "Gemini" && it.isActive && it.apiKey.isNotEmpty() }
                ...
                val dietHistory = dietRepository.totalCaloriesToday.value // Simplified context
